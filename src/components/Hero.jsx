...
const tagVariants = {
  ...
  visible: (i) => ({ opacity: 1, y: 0, transition: { delay: 0.2 + i * 0.08, duration: 0.5, type: 'spring' } })
}
...
const Hero = () => {
  ...
  // For matching OVID/INTERNATIONAL width
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-white dark:bg-[#10102a]">
      {/* Softer, Pastel Animated Blobs */}
      <div className="absolute inset-0">
        <div className="absolute top-16 right-10 w-80 h-80 bg-gradient-to-r from-pink-300 to-orange-200 rounded-full blur-3xl opacity-20 animate-pulse"></div>
        <div className="absolute bottom-32 left-16 w-72 h-72 bg-gradient-to-r from-purple-300 to-indigo-200 rounded-full blur-3xl opacity-20 animate-pulse delay-1000"></div>
        <div className="absolute top-1/3 left-10 w-64 h-64 bg-gradient-to-r from-orange-200 to-red-200 rounded-full blur-3xl opacity-20 animate-pulse delay-2000"></div>
        <div className="absolute bottom-16 right-1/4 w-56 h-56 bg-gradient-to-r from-green-200 to-emerald-200 rounded-full blur-3xl opacity-20 animate-pulse delay-3000"></div>
        <div className="absolute top-20 left-1/3 w-48 h-48 bg-gradient-to-r from-pink-200 to-rose-200 rounded-full blur-3xl opacity-20 animate-pulse delay-1500"></div>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 px-4 md:px-8">
        ...
        <div className="flex-1 flex flex-col items-start justify-center py-12 w-full space-y-4 md:space-y-6">
          ...
          {/* Supporting Text */}
          <div className="text-lg md:text-xl text-cyan-700 dark:text-cyan-200 font-medium mb-2">Driven by Design. Powered by Innovation.</div>
          {/* Stained Glass Tags (Networking, Data Recovery, 3D Design, Cloud Systems, Automation, Low-Code Apps) */}
          ...
          >
            ...
            <h2 className="text-2xl md:text-4xl font-bold text-gray-900 dark:text-white text-center leading-tight">
              ...
              <span className="inline-block w-3 h-7 align-middle">
                <span className="inline-block animate-pulse bg-gray-900 dark:bg-white rounded-sm w-1 h-6 ml-1 align-middle" style={{ verticalAlign: 'middle' }}></span>
              </span>
            </h2>
            {/* Subheading */}
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full justify-center mt-4">
              <button
                className="px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white dark:text-white font-semibold rounded-full shadow-lg hover:shadow-cyan-500/25 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-cyan-400 animate-pulse"
...
@keyframes blob1 {
  ...
  20% { transform: translate(-20px, 10px) scale(1.04); }
  40% { transform: translate(15px, -10px) scale(0.98); }
  60% { transform: translate(-10px, 15px) scale(1.02); }
  80% { transform: translate(10px, -15px) scale(1.01); }
}
@keyframes blob2 {
  0%, 100% { transform: translate(0,0) scale(1); }
  25% { transform: translate(18px, 8px) scale(1.05); }
  50% { transform: translate(-12px, -14px) scale(0.97); }
  75% { transform: translate(8px, 12px) scale(1.03); }
}
@keyframes blob3 {
  0%, 100% { transform: translate(0,0) scale(1); }
  30% { transform: translate(-15px, 18px) scale(1.06); }
  60% { transform: translate(12px, -16px) scale(0.99); }
  90% { transform: translate(-8px, 10px) scale(1.02); }
}
@keyframes blob4 {
  0%, 100% { transform: translate(0,0) scale(1); }
  35% { transform: translate(22px, -18px) scale(1.07); }
...
.animate-blob1 { animation: blob1 32s ease-in-out infinite alternate; }
.animate-blob2 { animation: blob2 36s ease-in-out infinite alternate; }
.animate-blob3 { animation: blob3 30s ease-in-out infinite alternate; }
.animate-blob4 { animation: blob4 34s ease-in-out infinite alternate; }
.animate-blob5 { animation: blob5 28s ease-in-out infinite alternate; }
`}</style>
    </section>
  )
}
